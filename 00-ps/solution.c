...
size_t ParseDataFromFile(const char* path, char*** arr) {
    ...
    *arr = (char** )malloc((elem_pos + 1) * sizeof(char*));
    (*arr)[elem_pos] = NULL;
    for (size_t i = 0; i < elem_pos; ++i) {
        (*arr)[i] = (char* )malloc((strlen(tmp_arr[i]) + 1) * sizeof(char));
        memcpy((*arr)[i], tmp_arr[i], strlen(tmp_arr[i]) + 1);
    }
	FreeDoubleArray(tmp_arr, cur_arr_len);
    close(fd);
    return elem_pos + 1;
